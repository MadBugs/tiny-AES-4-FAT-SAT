...
/*static*/ void Cipher(state_t* state, uint8_t RoundKey[AES_KEYEXPSIZE])
{
  ...
  const char *pch;
  pch="\t<<HWSEC00 <<<<<:%016llx%016llx *** BEFORE...AddRoundKey(round=0) <-- state\n";
  ...
  // ---------------------------------
#ifdef CIPHER_PRINTS_
  pch="\t<<HWSEC01 <<<<<:%016llx%016llx *** AFTER....AddRoundKey(round=0) <-- state\n";
  px128_SWP(pch, (void*)p); 
#endif 
...
#endif 
  {
    ...
    // ---------------------------------
#ifdef CIPHER_PRINTS_
    pch="\t<<HWSEC02 <<<<<:%016llx%016llx *** AFTER....SubBytes(          ) <-- state\n";
    px128_SWP(pch, (void*)p); 
#endif 
    ...
    // ---------------------------------
#ifdef CIPHER_PRINTS_
    pch="\t<<HWSEC03 <<<<<:%016llx%016llx *** AFTER....Shif4Rows(         ) <-- state\n";
    px128_SWP(pch, (void*)p); 
#endif 
    ...
    // ---------------------------------
#ifdef CIPHER_PRINTS_
    pch="\t<<HWSEC04 <<<<<:%016llx%016llx *** AFTER....MixColumns(        ) <-- state\n";
    px128_SWP(pch, (void*)p); 
#endif 
    ...
    // ---------------------------------
#ifdef CIPHER_PRINTS_
    pch="\t<<HWSEC05 <<<<<:%016llx%016llx *** AFTER....AddRoundKey(round=1) <-- state\n";
    px128_SWP(pch, (void*)p); 
#endif 
  }
...
/*static*/ void InvCipher(state_t* state, uint8_t RoundKey[AES_KEYEXPSIZE])
{
  ...
  AddRoundKey(nRounds, state, RoundKey); 
...
#ifdef ROUND_1_ONLY
    round = 1;
    // HWSEC: Let's call the state at this initial point state{0}
    // HWSEC: It corresponds to the ciphertext leaked after a one 
    // HWSEC: round AES-128 encryptation (when round was 1)

    //uint64_t *p =(uint64_t *) (*state);    
    //pch=">>HWSEC05 >>>>>:%016llx%016llx *** BEFORE...AddRoundKey(round=1) <-- state\n";
    //px128_SWP(pch, (void*)p); 
    // 05-------------------------------
    AddRoundKey(round, state, RoundKey);
    // ---------------------------------
    ...
    //pch=">>HWSEC04 *RK1*:%016llx%016llx *** AFTER....AddRoundKey(round=1) <-- RoundKey(1) **HACK** \n";
    //PEN_YELLOW(); px128_SWP(pch, (void*)rk1); PEN_NC();

    //pch=">>HWSEC04 >>>>>:%016llx%016llx *** AFTER....AddRoundKey(round=1) <-- state\n";
    //px128_SWP(pch, (void*)p); 

    // 04-------------------------------
    InvMixColumns(state);
    // ---------------------------------
    //pch=">>HWSEC03 >>>>>:%016llx%016llx *** AFTER....InvMixColumns(     ) <-- state\n";
    //px128_SWP(pch, (void*)p); 

    // 03-------------------------------
    InvShiftRows(state);
    //pch=">>HWSEC02 >>>>>:%016llx%016llx *** AFTER....InvShiftRows(      ) <-- state\n";
    //px128_SWP(pch, (void*)p); 
    // ---------------------------------

    // 02-------------------------------
    InvSubBytes(state);
    //pch=">>HWSEC01 >>>>>:%016llx%016llx *** AFTER....InvSubBytes(       ) <-- state\n";
    //px128_SWP(pch, (void*)p); 
    // ---------------------------------
  ...
  AddRoundKey(0, state, RoundKey); // Round 0
#ifdef ROUND_1_ONLY
  //pch=">>HWSEC00 >>>>>:%016llx%016llx *** AFTER....AddRoundKey(round=0) <-- state\n";
  //px128_SWP(pch, (void*)p); 
#endif
  // ---------------------------------
} /* End of InvCipher() */
